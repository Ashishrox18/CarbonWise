...
import { EcoChallenge, EmissionCategory } from '@/types';
 ...
 * category, then filling with a diverse mix.
 *
 * @param largestCategory - The category contributing most to today's footprint.
 * @param date - ISO date string used as a deterministic seed for selection.
 * @returns Three challenges for the day.
 */
export function selectDailyChallenges(
  ...
  // Always include one challenge from the largest category
  const primary = ALL_CHALLENGES.find(c => c.category === largestCategory)
    ?? ALL_CHALLENGES[0];

  // Fill remaining slots with variety (no duplicates)
  const others = ALL_CHALLENGES
    .filter(c => c.id !== primary.id)
    .sort((a, b) => {
      // Use deterministic rotation so challenges vary by date
